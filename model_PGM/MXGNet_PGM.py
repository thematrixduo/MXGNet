...
class Model(nn.Module):
    def __init__(self,A,B,  device='cuda',num_fl_1d=5,batch_size = 64,beta=10):
        ...
        self.relation_conv = ResConvReason(BasicBlock,[1,2],[48,self.rel_size],in_dim=self.enc_size*3,g_dim = self.g_size)
        self.infer_conv = ResConvInfer(BasicBlock,[2],[128],256,in_dim=self.rel_size*10)

        self.rel_local_fc_1 = nn.Linear(self.enc_size*2+self.num_fl*2,self.g_size)
        self.rel_local_fc_1_bn = nn.BatchNorm1d(self.g_size)

        self.rel_local_fc = nn.Linear(2*self.g_size,self.g_size)
        self.rel_local_fc_bn = nn.BatchNorm1d(self.g_size)

        self.dropout = nn.Dropout(0.5)
        self.infer_fc = nn.Linear(256,8)
        self.meta_conv = nn.Conv2d(self.rel_size,12,kernel_size=5)
    ...
    def forward_actual(self,x):
        ...
        r_v_1 = self.relation_infer(f_g_list[0],f_g_list[3],f_g_list[6],f_l_list[0],f_l_list[3],f_l_list[6])
        r_v_2 = self.relation_infer(f_g_list[1],f_g_list[4],f_g_list[7],f_l_list[1],f_l_list[4],f_l_list[7]) 
        score_list = []
        r_h_list =[r_h_1,r_h_2]
        r_v_list =[r_v_1,r_v_2]       
  
        r_sum = r_h_1 + r_h_2 + r_v_1 + r_v_2
        ...
        for i in range(8):
            ...
            r_v_3 = self.relation_infer(f_g_list[2],f_g_list[5],f_g_list[8+i],f_l_list[2],f_l_list[5],f_l_list[8+i]) 
            r_h_list.append(r_h_3)
            r_v_list.append(r_v_3) 
      
        h_cb = self.infer_conv(torch.cat(r_h_list,1)).squeeze()
        v_cb = self.infer_conv(torch.cat(r_v_list,1)).squeeze()        
        h_score = self.infer_fc(self.dropout(h_cb+v_cb))
